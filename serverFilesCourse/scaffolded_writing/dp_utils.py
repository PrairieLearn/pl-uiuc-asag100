...
ConstraintT = Callable[[DPStudentSubmission], Optional[str]]


def declare_func_constraint() -> ConstraintT:
    def constraint(submission: DPStudentSubmission) -> Optional[str]:
    ...
    def constraint(submission: DPStudentSubmission) -> Optional[str]:
        ...
        if unexplained_params:
            if len(unexplained_params) == 1:
                return f"Your function takes {list_to_english(sorted(unexplained_params))} as an input parameter, but your subproblem definition does not explain how this parameter affects the output of the function."
            else:
                return f"Your function takes {list_to_english(sorted(unexplained_params))} as input parameters, but your subproblem definition does not explain how these parameters affect the output of the function."
        if undefined_variables:
            if len(undefined_variables) == 1:
                return f"Your subproblem definition refers to the variable {list_to_english(sorted(undefined_variables))}, which is undefined. You should only refer to variables which are defined in the original problem or declared as input parameters to your function."
            else:
                return f"Your subproblem definition refers to the variables {list_to_english(sorted(undefined_variables))}, which are undefined. You should only refer to variables which are defined in the original problem or declared as input parameters to your function."
...
def reduces_recursively_constraint(
    ...
    if not field_requiring_parameters.isupper():
        raise ValueError("field_requiring_paramters was not a CFG Rule token")
