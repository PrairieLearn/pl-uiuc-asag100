...
def correct_object(objects: Set[str], *, isGprime: bool = False) -> ConstraintT:
    graph, var = ("STATEMENT_ABOUT_G_PRIME", "G'") if isGprime else ("STATEMENT_ABOUT_G", "G")
...
def defined_simple_terminal_node(correct_node: str, *, isEnd: bool = False, isGprime: bool = False, feedback: str = '') -> ConstraintT:
    graph, var = ("STATEMENT_ABOUT_G_PRIME", "G'") if isGprime else ("STATEMENT_ABOUT_G", "G")
    terminal, node = ("END_NODE", 'end') if isEnd else ("START_NODE", 'start')

    def constraint(submission: GraphStudentSubmission) -> Optional[str]:
        if submission.does_path_exist(graph, "OBJECT", "DESCRIPTION", terminal, correct_node):
            return None
...
def defined_general_terminal_node(*, isEnd: bool = False, isGprime: bool = False) -> ConstraintT:
    graph, var = ("STATEMENT_ABOUT_G_PRIME", "G'") if isGprime else ("STATEMENT_ABOUT_G", "G")
    terminal, node = ("END_NODE", 'end') if isEnd else ("START_NODE", 'start')
    general = "T_GENERAL" if isEnd else "S_GENERAL"

    def constraint(submission: GraphStudentSubmission) -> Optional[str]:
        if not submission.does_path_exist(graph, "OBJECT", "DESCRIPTION", terminal, general):
