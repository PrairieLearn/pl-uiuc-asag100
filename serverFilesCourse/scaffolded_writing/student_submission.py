...
class StudentSubmission():
    ...
    def __post_init__(self) -> None:
        try:
            possible_parse_trees = RecursiveDescentParser(self.cfg).parse_all(self.token_list)
        except ValueError:
            raise PARSE_ERROR
    ...
    def does_path_exist(self, *path: str) -> bool:
        ...
        def does_path_exist_starting_from_node(path_index: int, node: Union[Tree, str]) -> bool:
            # If string case, must be at the end of the path
            if isinstance(node, str):
                return path_index == len(path_list)-1 and path_list[path_index] == node
            ...
            return any(does_path_exist_starting_from_node(path_index+1, child) for child in node)
